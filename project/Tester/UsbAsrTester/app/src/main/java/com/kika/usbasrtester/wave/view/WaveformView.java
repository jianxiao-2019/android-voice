...
package com.kika.usbasrtester.wave.view;
...
import com.kika.usbasrtester.wave.utils.SoundFile;
...
public class WaveformView extends View {
    ...
    public void setSoundFile(SoundFile soundFile) {
        ...
        mTotoalWidth = (int)((float)maxPos()*1.1);
    }
    ...
    public void zoomIn() {
        if (canZoomIn()) {
            ...
            mSelectionEnd *= 2;
            mHeightsAtThisZoomLevel = null;
            int offsetCenter = mOffset + getTotalWidth() / 2;
            offsetCenter *= 2;
            mOffset = offsetCenter - getTotalWidth() / 2;
            if (mOffset < 0)
                mOffset = 0;
            invalidate();
        }
    ...
    protected void drawWaveformLine(Canvas canvas,
    	...
    	int pos = maxPos();
    	float rat =((float)getTotalWidth()/pos);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        int measuredWidth = getTotalWidth();
...
//            width = measuredWidth;
        ...
        for (i = 0; i < maxPos(); i++) {
            ...
            if (i + start == mPlaybackPos && playFinish != 1) {
     	        canvas.drawCircle(i*getTotalWidth()/maxPos(), line_offset/4, line_offset/4, circlePaint);// 上圆
     	        canvas.drawCircle(i*getTotalWidth()/maxPos(), getMeasuredHeight()-line_offset/4, line_offset/4, circlePaint);// 下圆
     	        canvas.drawLine(i*getTotalWidth()/maxPos(), 0, i*getTotalWidth()/maxPos(), getMeasuredHeight(), circlePaint);//垂直的线
            	//画正在播放的线
                //canvas.drawLine(i*getTotalWidth()/maxPos(), 0, i*getTotalWidth()/maxPos(), measuredHeight, paint);
            }
        }

        this.setMinimumWidth(measuredWidth);
    ...
    final private int getTotalWidth() {
        return mTotoalWidth;
