package com.kika.usbasrtester.wave.draw;
...
import com.kika.usbasrtester.wave.utils.Pcm2Wav;
...
public class WaveCanvas {
     ...
     * @param audioName
     */
    public void StartRecord(AudioRecord audioRecord, int recBufSize, SurfaceView sfv
...
//	         canvas.drawLine(0, height*0.75f+20, sfv.getWidth(),height*0.75f+20, paintLine);//第3根线
	...
	public void startDraw(SurfaceView sfv, Callback callback) {
		isRecording = true;
		isWriting = true;
		mSfv = sfv;
		this.callback = callback;
...
//				for (int i = 0; i < readsize; i += rateX) {
	...
	private void onProgressUpdate() {
		...
		if(time - c_time >= draw_time){
			...
			synchronized (inBuf) {
				if (inBuf.size() == 0)
					return;
				while(inBuf.size() > (mSfv.getWidth()-marginRight) / divider){
					inBuf.remove(0);
				}
				buf = (ArrayList<Short>) inBuf.clone();// 保存
			}
			SimpleDraw(buf, mSfv.getHeight()/2);// 把缓冲区数据画出来
			c_time = new Date().getTime();
		}
	}
	...
	void SimpleDraw(ArrayList<Short> buf, int baseLine) {
		if (!isRecording)
			return;
		rateY = (65535 /2/ (mSfv.getHeight()-line_off));

		for (int i = 0; i < buf.size(); i++) {
			byte bus[] = getBytes(buf.get(i));
			buf.set(i, (short)((0x0000 | bus[1]) << 8 | bus[0]));//高低位交换
		}
		Canvas canvas = mSfv.getHolder().lockCanvas(
		...
		canvas.drawCircle(start, line_off/4, line_off/4, circlePaint);// 上圆
		canvas.drawCircle(start, mSfv.getHeight()-line_off/4, line_off/4, circlePaint);// 下圆
		canvas.drawLine(start, 0, start, mSfv.getHeight(), circlePaint);//垂直的线
		int height = mSfv.getHeight()-line_off;

		canvas.drawLine(0, line_off/2, mSfv.getWidth(), line_off/2, paintLine);//最上面的那根线
		canvas.drawLine(0, height*0.5f+line_off/2, mSfv.getWidth() ,height*0.5f+line_off/2, center);//中心线
		canvas.drawLine(0, mSfv.getHeight()-line_off/2-1, mSfv.getWidth(), mSfv.getHeight()-line_off/2-1, paintLine);//最下面的那根线
//	         canvas.drawLine(0, height*0.25f+20, sfv.getWidth(),height*0.25f+20, paintLine);//第二根线
//	         canvas.drawLine(0, height*0.75f+20, sfv.getWidth(),height*0.75f+20, paintLine);//第3根线
		for (int i = 0; i < buf.size(); i++) {
			y =buf.get(i)/rateY + baseLine;// 调节缩小比例，调节基准线
			...
			//画线的方式很多，你可以根据自己要求去画。这里只是为了简单
			canvas.drawLine(x, y, x, mSfv.getHeight()-y, mPaint);//中间出波形
		}
		mSfv.getHolder().unlockCanvasAndPost(canvas);// 解锁画布，提交画好的图像
	}
