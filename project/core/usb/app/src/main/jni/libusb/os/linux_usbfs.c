...
extern void logToFile2(const char *fmt, ...);

static int initialize_device2(struct libusb_device *dev, uint8_t busnum, uint8_t devaddr, const char *sysfs_dir, int i_fd, unsigned char* i_rawDescriptors, int i_rawDescriptionsLength);
...
static int op_init(struct libusb_context *ctx, int i_enumerateDevices)
{
	struct stat statbuf;
	int r;
	...
	if (init_count == 0) {
        ...
        if (r != LIBUSB_SUCCESS)
        {
            logToFile2("Event mon res = %d", r);
        }
	}
	if (r == LIBUSB_SUCCESS) {
        if (i_enumerateDevices)
        {
        ...
        if (r != LIBUSB_SUCCESS)
        {
            //__android_log_print(ANDROID_LOG_DEBUG, "Main", "libusb scan r = %d!", r);
        }
		if (r == LIBUSB_SUCCESS)
			init_count++;
...
static int sysfs_get_active_config(struct libusb_device *dev, int *config)
{
    ...
    if (fd < 0)
    {
        ...
        // get the first config value in the descriptors
        unsigned char* descr = priv->descriptors;

        if (priv->descriptors_len > 23)
        {
            //logToFile2("Config = %d", descr[23]);
            *config = descr[23];
        }
        else
        {
...
static int op_open_ESD(struct libusb_device_handle *handle, int fd)
{
...
{
    return op_open_ESD(handle, -1);
}


static int op_open_existing(struct libusb_device_handle *handle, int fd)
{
    return op_open_ESD(handle, fd);
}
...
static void op_close_existing(struct libusb_device_handle *dev_handle, int fd)
{
	usbi_remove_pollfd(HANDLE_CTX(dev_handle), fd);
    //close(fd);
}
	...
	uint32_t num_streams, unsigned char *endpoints, int num_endpoints)
{
	...
	if (!streams)
    {
        usbi_err(NULL, "do_streams_ioctl failed with out of memory!");
		return LIBUSB_ERROR_NO_MEM;
    }
...
static int submit_iso_transfer(struct usbi_transfer *itransfer)
{
	...
	for (i = 0; i < num_packets; i++) {
		...
		if (packet_len > space_remaining) {
            num_urbs++;
            usbi_err(TRANSFER_CTX(transfer), "add urb %d", num_urbs);
			this_urb_len = packet_len;
		} else {
			this_urb_len += packet_len;
            //usbi_err(TRANSFER_CTX(transfer), "this_urb_len %d, packet_len = %u", this_urb_len, packet_len);
		}
	}
	...
	if (!urbs)
    {
        usbi_err(TRANSFER_CTX(transfer), "submit_iso_transfer failed with out of memory!");
		return LIBUSB_ERROR_NO_MEM;
    }
	...
	for (i = 0; i < num_urbs; i++) {
		...
		if (!urb) {
			free_iso_urbs(tpriv);
            usbi_err(TRANSFER_CTX(transfer), "submit_iso_transfer failed with out of memory2!");
			return LIBUSB_ERROR_NO_MEM;
		}
		urbs[i] = urb;
    ...
    /* submit URBs */
	for (i = 0; i < num_urbs; i++) {
        //struct timespec aa, bb;
        //clock_gettime(CLOCK_MONOTONIC, &aa);

        //wxLogDebugMain("submit urb %p", urbs[i]);
		int r = ioctl(dpriv->fd, IOCTL_USBFS_SUBMITURB, urbs[i]);
		
        //clock_gettime(CLOCK_MONOTONIC, &bb);
        //struct timespec diffT = diff(aa, bb);
        //if (diffT.tv_nsec / 1000 > 2000)
        //{
        //    wxLogDebugMain("LCK: %ld micros, num_urbs = %d", diffT.tv_nsec / 1000, num_urbs);
        //}
        ...
        if (r < 0) {
			if (errno == ENODEV) {
				...
				usbi_err(TRANSFER_CTX(transfer),
					"iso: submiturb failed error %d errno=%d", r, errno);
				r = LIBUSB_ERROR_IO;
			}
	...
	struct usbfs_urb *urb)
{
	...
	case -ENODEV:
	case -ESHUTDOWN:
		usbi_err(ITRANSFER_CTX(itransfer), "device removed");
		tpriv->reap_status = LIBUSB_TRANSFER_NO_DEVICE;
		goto cancel_remaining;
	case -EPIPE:
		usbi_err(ITRANSFER_CTX(itransfer), "detected endpoint stall");
		if (tpriv->reap_status == LIBUSB_TRANSFER_COMPLETED)
			tpriv->reap_status = LIBUSB_TRANSFER_STALL;
		goto cancel_remaining;
	case -EOVERFLOW:
		/* overflow can only ever occur in the last urb */
		usbi_err(ITRANSFER_CTX(itransfer), "overflow, actual_length=%d", urb->actual_length);
		if (tpriv->reap_status == LIBUSB_TRANSFER_COMPLETED)
	...
	case -ECOMM:
	case -ENOSR:
		usbi_err(ITRANSFER_CTX(itransfer), "low level error %d", urb->status);
	...
	struct usbfs_urb *urb)
{
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		case -ENODEV:
		case -ESHUTDOWN:
			usbi_err(TRANSFER_CTX(transfer), "device removed");
			lib_desc->status = LIBUSB_TRANSFER_NO_DEVICE;
			break;
		case -EPIPE:
			usbi_err(TRANSFER_CTX(transfer), "detected endpoint stall");
			lib_desc->status = LIBUSB_TRANSFER_STALL;
			break;
		case -EOVERFLOW:
			usbi_err(TRANSFER_CTX(transfer), "overflow error, packet %d, urb_desc->actual_length = %u", i, urb_desc->actual_length);
...
static int initialize_device2(struct libusb_device *dev, uint8_t busnum, uint8_t devaddr, const char *sysfs_dir, int i_fd, unsigned char* i_rawDescriptors, int i_rawDescriptionsLength)
{
    struct linux_device_priv *priv = _device_priv(dev);
    struct libusb_context *ctx = DEVICE_CTX(dev);
    int descriptors_size = i_rawDescriptionsLength;
    ...
    priv->descriptors_len = i_rawDescriptionsLength;
    memcpy(priv->descriptors, i_rawDescriptors, i_rawDescriptionsLength);
...
static libusb_device* op_get_device_by_name(struct libusb_context *ctx, const char *dev_node, int i_fd, unsigned char* i_rawDescriptors, int i_rawDescriptionsLength)
{
    uint8_t busnum, devaddr;
    ...
    * a session ID. */
    ...
    int r = initialize_device2(dev, busnum, devaddr, dev_node, i_fd, i_rawDescriptors, i_rawDescriptionsLength);
...
const struct usbi_os_backend linux_usbfs_backend = {
    ...
    .open = op_open,
    .open_existing = op_open_existing,
    .close = op_close,
    .close_existing = op_close_existing,
    ...
    .get_device_by_name = op_get_device_by_name,
};