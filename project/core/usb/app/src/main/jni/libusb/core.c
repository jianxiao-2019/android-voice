...
void logToFile2(const char *fmt, ...);
#endif
 ...
 */
int API_EXPORTED libusb_open_ESD(libusb_device *dev,
...
int API_EXPORTED libusb_open(libusb_device *dev,
	libusb_device_handle **handle)
{
    return libusb_open_ESD(dev, handle, -1);
}


int API_EXPORTED libusb_open_existing(libusb_device *dev,
	libusb_device_handle **handle, int fd)
{
    return libusb_open_ESD(dev, handle, fd);
}
 ...
 */
void API_EXPORTED libusb_close_ESD(libusb_device_handle *dev_handle, int fd)
{
...
DEFAULT_VISIBILITY
libusb_device * LIBUSB_CALL libusb_get_device_by_name(libusb_context *ctx, const char *dev_name, int i_fd, unsigned char* i_rawDescriptors, int i_rawDescriptionsLength)
{
    if (usbi_backend->get_device_by_name == NULL)
    {
        /* Not supported on this platform */
        return NULL;
    }

    return usbi_backend->get_device_by_name(ctx, dev_name, i_fd, i_rawDescriptors, i_rawDescriptionsLength);
}
	...
	int interface_number, int alternate_setting)
{
	...
	if (!(dev->claimed_interfaces & (1 << interface_number))) {
		usbi_mutex_unlock(&dev->lock);
        usbi_err(NULL, "LIBUSB_ERROR_NOT_FOUND: dev->claimed_interfaces = %x, interface_number = %x", dev->claimed_interfaces, interface_number);
 ...
 * \see contexts
 */
int API_EXPORTED libusb_init(libusb_context **context, int i_enumerateDevices)
{
	...
	if (usbi_backend->init) {
		r = usbi_backend->init(ctx, i_enumerateDevices);
		if (r)
			goto err_free_ctx;
	}
	...
	const char *function, const char *format, va_list args)
{
	...
	case LIBUSB_LOG_LEVEL_INFO:
		prefix = "info";
        logToFile("LIBUSB INFO: ", format, args);
		break;
...
void logToFile(const char* pre, const char *format, va_list args)
{
    if (s_libusb_logMethod == 1 && s_libusb_logFileName != NULL)
    {
        ...
        {
            __android_log_print(ANDROID_LOG_DEBUG, "Main", "couldn't open logfile %s!", s_libusb_logFileName);
        }
    }
}
