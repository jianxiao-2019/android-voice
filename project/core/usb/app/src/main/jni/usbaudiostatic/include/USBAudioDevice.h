 ...
 * \param i_userData any user data that was passed with setRecordCallback()
 */
...
#ifdef SUPPORTS_TRACK
  : public AEMAudioDevice
#else
  : public AudioDevice
#endif
{
public:
	USBAudioDevice(USBAudioManager* i_USBAudioManager,
                   libusb_device *i_device,
		           int i_fd,
                   int i_productId,
                   int i_vendorId,
     ...
     */
    bool init(bool *o_askResetDevice, bool i_disableHID, int& o_stackTrace, bool i_findInputs, bool i_findMidiInputs, bool i_findMidiOutputs, bool i_findInterruptInputs);

    /**
    * \brief On Android 7 and higher, we cannot determine the actual bus speed anymore. Normally, letting the software determine it
    *        by looking at some properties of the device is sufficient, but sometimes it fails to choose the correct one and the user
    *        should set the speed.
    */
    void setBusSpeedEstimation(BusSpeedEstimation i_busSpeedEstimation);
    ...
    USBConfiguration *getActiveConfig() const;
    ...
    void findAndAddInterruptEndPoints(bool i_in,
                                      std::vector< USBInterruptConfig* >& o_configs);
    ...
    /**
    * \brief Start the MIDI input transfers. Make sure not to call startAllTransfers() with i_recordMidi being true.
    */
    bool startMIDIInputTransfers();

    /// Stops all running MIDI input transfers and waits for them to finish
    void stopMIDIInputTransfers();


    /**
    * \brief Start audio in and/or out transfers.
    */
    bool startAudioTransfers(bool i_playAudio, bool i_recordAudio);

    /// Stops all running audio transfers and waits for them to finish. Returns measured audio latency in case of audio recording
    int stopAudioTransfers();
    ...
    std::vector< USBInterruptConfig* >& getInterruptInputStreamConfigs();
    ...
    void cancelAllInterruptTransfers();

    void setAltSettingZeroAfterStop(bool i_enable);
    void deviceRemoved();

    int getFd();

    virtual int getTypeNr() const { return 10; }

    /// 1 or 2
    int getUSBAudioVersion() const;

    /// Starts a thread on the 2nd core doing nothing other than NOP's to keep the CPU frequency high
    /// Most if not all multi-core chipsets use the same clock frequency for at least the first 2 cores.
    /// The USB thread is asked to run on core 0 to not interfere with the booster thread.
    virtual void setUseCPUBooster(bool i_useCPUBooster);
...
private:
    ...
    void findMIDIEndPoints(bool i_findMidiInputs, bool i_findMidiOutputs);
    void findInterruptEndPoints();
    ...
    std::string getIOName(bool i_input, int i_index, int i_matchSampleRate) const;
    ...
    std::vector< USBInterruptConfig* > m_interruptInputStreamConfigs;
    std::vector< USBInterruptConfig* > m_interruptOutputStreamConfigs;
    ...
    bool m_setAltSettingZeroAfterStop;

    BusSpeedEstimation m_busSpeedEstimation;
    USBAudioManager* m_USBAudioManager;
    bool m_useCPUBooster;
    bool m_wasCPUBoosterRunning;
};
