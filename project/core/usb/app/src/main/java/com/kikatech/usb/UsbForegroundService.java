 ...
 */
public class UsbForegroundService extends Service implements USBDeviceManager.IUsbDriverListener {
    ...
    private void handleStartAudioTransfer(Intent intent) {
        try {
            AudioUtil.getIns().playSilence(UsbForegroundService.this);
    ...
    private static void launchCommend(Context context, String action, Bundle args) {
        try {
            ...
            Intent notifyIntent = new Intent(appCtx, UsbForegroundService.class);
    ...
    private Notification getForegroundNotification() {
        Intent closeIntent = new Intent(UsbForegroundService.this, UsbForegroundService.class);
        closeIntent.setAction(Commands.STOP_FOREGROUND);
        PendingIntent closePendingIntent = PendingIntent.getService(UsbForegroundService.this, SERVICE_ID, closeIntent, PendingIntent.FLAG_UPDATE_CURRENT);

        return new NotificationCompat.Builder(UsbForegroundService.this)
