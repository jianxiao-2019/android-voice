...
public abstract class BaseIMObject extends Chatable {
    ...
    private void processNotificationInfos(StatusBarNotification statusBarNotification) {
        if (LogUtil.DEBUG) {
            LogUtil.logd(TAG, "[processNotificationInfos] Start ---------- ---------- ---------- ----------");
        }
        ...
        if (LogUtil.DEBUG) {
            ...
            for (String key : extras.keySet()) {
                allKeys.append("[").append(key).append("], ");
            }
            if (allKeys.length() > 2) {
                allKeys.replace(allKeys.length() - 2, allKeys.length() - 1, "");
            }
            LogUtil.logd(TAG, String.format("allKeys: %s", allKeys.toString()));
        }
        ...
        if (LogUtil.DEBUG) {
            LogUtil.logd(TAG, "[processNotificationInfos] Ended ---------- ---------- ---------- ----------");
        }
    }
    ...
    public String getGroupName() {
        return groupName != null ? groupName : combinedGroupName;
    }
    ...
    private void printNotificationInfos() {
        try {
                        ...
                        case EXTRAS_TEXT_LINES:
                            ...
                            if (textLines != null) {
                                for (CharSequence textLine : textLines) {
                                    if (textLine != null) {
                                        LogUtil.logv(TAG, "[NotificationInfo][extras] key: " + key + ", value: " + textLine.toString());
                                    }
                                }
                            }
                            break;
