...
public class IMMessenger extends BaseIMObject {
    ...
    protected void processNotificationContent() {
        if (TextUtils.isEmpty(tag) || (!tag.startsWith(NOTIFICATION_TAG_GROUP) && !tag.startsWith(NOTIFICATION_TAG_USER))) {
            return;
        }
         ...
         */
        if (tag.startsWith(NOTIFICATION_TAG_GROUP)) // Group
        {
            /** syntax
             *
             * title: {groupName}
        ...
        } else if (tag.startsWith(NOTIFICATION_TAG_USER)) // User
        {
            /** syntax
             * title: {userName}
             * speak: {userName} : {msg}
            ...
            groupName = null;
            if (LogUtil.DEBUG) {
                LogUtil.log(TAG, String.format("text: %1$s, isEmpty: %2$s", text, TextUtils.isEmpty(text)));
            }
            if (!TextUtils.isEmpty(text)) {
